...
3. [Beyond map()](#beyond-map)
    1. [Modifying and summarizing vectors/lists](#modifying-and-summarizing-vectors/lists)
        1. [keep() and discard()](#keep-and-discard)
        2. [map_if()](#map_if)
        3. [every() and some()](#every-and-some)
    2. [Modifying functions](#modifying-functions)
        1. [negate()](#negate)
        2. [partial()](#partial)
...
`negate()` ... negates a predicate function (aren't the `purrr` function names done well?). For example, let's say you want to check which elements of a list were not null. This is how you would do it with `map_lgl` (which returns a logical vector rather than a list):
