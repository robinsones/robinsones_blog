...
features_by_season_half <- nb_features_by_season %>%
  ...
  ungroup() %>%
  mutate(pct_featuring = ifelse(season_half == "first", nb_paintings / (15 * 67), nb_paintings / (16 * 67))) %>%
  select(-nb_paintings) %>%
  spread(season_half, pct_featuring) 

features_by_season_half %>%
  mutate(difference = `Second half` - `First half`) %>%
  top_n(10, abs(difference)) %>%
  mutate(feature = fct_reorder(feature, `First half`)) %>%
  ggplot(aes(y = feature, x = `First half`, xend = `Second half`)) + 
  geom_point()
  ggalt::geom_dumbbell(size = 2,
                    color="#65ADC2", 
                    colour_x = "#E84646", colour_xend = "#233B43") + 
  # needed to add legend to plot
  labs(x = "", y = "") + 
...
nb_features_by_season %>%
  ...
  summarize(nb_paintings = sum(nb_paintings)) %>%
  ungroup() %>%
  mutate(nb_paintings = ifelse(season_half == "first", nb_paintings / (15 * 67), nb_paintings / (16 * 67))) %>%
...
I highly recommend reading Julia Silge's great post on [Principle Components Analysis](https://juliasilge.com/blog/stack-overflow-pca/), where she explains it using StackOverflow data. In short, Principle Components Analysis is a dimensionality-reduction technique that takes all of your variables and creates a new set of characteristics that still contains the most information (check out [this StackOverflow answer](https://stats.stackexchange.com/questions/2691/making-sense-of-principal-component-analysis-eigenvectors-eigenvalues) for great explanations from simple to mathematical).

We'll start by selecting only features that appear in at least 5 paintings and calculate three principle components, adding `scale = TRUE` to scale the features. This is a critical step; while the features are all on the same scale (0 or 1, not say 0 or 100 for one and 0 or 50 for another), they have different variances as some have a lot of 1s and some a lot of 0s. We need to center our data so that they all have the same mean and variance.
...
After  take a look at which features have the most relative importance (highest absolute value) for each. We're going to use the new functions `tidytext` functions `reorder_within()` and `scale_x_reordered()` to make the features ordered within each plot (see Julia Silge's [post](https://juliasilge.com/blog/reorder-within/) for more).
...
tidied_pca %>%
  ...
  labs(title = "What distinguishes Bob Ross paintings?",
       subtitle = "They're divided between trees vs. beach, structures vs. mountains, and water vs. winter structures", y = "Relative importance in principle component",
       x = "")
```

We find that the first principle component divides painting between those at the ocean (with beach and palm trees) and those in the tree covered mountains. The next component divides mountains from structures like cabin and barns, while the final one distinguishes a cabin in the snow from a grassy river. 
