...
These features were in every season - were any other features? 
...
Yes - 6 more were in at least one painting, including grass, lake, and river.  
...
```{r}
tags_pca <- irlba::prcomp_irlba(bob_ross_for_cluster, n = 3, scale. = TRUE)
```

```{r}
tidied_pca <- bind_cols(feature = colnames(bob_ross_for_cluster),
                        as_tibble(tags_pca$rotation)) %>%
  gather(PC, value, PC1:PC3)
```

We're going to use the new functions `reorder_within()` and `scale_x_reordered()` to make the features ordered within each plot (see Julia Silge's [post](https://juliasilge.com/blog/reorder-within/) for more).
...
tibble(k = 1:50) %>%
  mutate(tot_withinss = map_dbl(k, ~ kmeans(scale(bob_ross_for_cluster), centers = .)$tot.withinss)) %>%
...
```{r}
model <- kmeans(scale(bob_ross_for_cluster), 9)
